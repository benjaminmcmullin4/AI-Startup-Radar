...
from db import get_all_theses, insert_thesis, update_thesis, get_default_thesis
from pipeline import score_all_companies
from schema import ThesisCriteria
...
def render_thesis_builder():
    ...
    st.caption(
        "Configure scoring weights and investment criteria, then score all "
        "companies against your thesis."
    )
    ...
    with st.form("thesis_form"):
        name = st.text_input(
            "Thesis Name",
            value=thesis["name"] if thesis else "My Growth Thesis",
        )
        ...
        with wc1:
            w_financial = st.slider(
                "Financial", 0, 100,
                int((thesis or {}).get("weight_financial", 0.25) * 100), step=5,
            )
        with wc2:
            w_team = st.slider(
                "Team", 0, 100,
                int((thesis or {}).get("weight_team", 0.25) * 100), step=5,
            )
        with wc3:
            w_market = st.slider(
                "Market", 0, 100,
                int((thesis or {}).get("weight_market", 0.20) * 100), step=5,
            )
        with wc4:
            w_product = st.slider(
                "Product", 0, 100,
                int((thesis or {}).get("weight_product", 0.15) * 100), step=5,
            )
        with wc5:
            w_momentum = st.slider(
                "Momentum", 0, 100,
                int((thesis or {}).get("weight_momentum", 0.15) * 100), step=5,
            )
        ...
        # Parse existing criteria using Pydantic
        if thesis:
            criteria_raw = thesis.get("criteria_json", "{}")
            if isinstance(criteria_raw, str):
                existing_criteria = ThesisCriteria.model_validate_json(criteria_raw)
            elif isinstance(criteria_raw, dict):
                existing_criteria = ThesisCriteria(**criteria_raw)
            else:
                existing_criteria = ThesisCriteria()
        else:
            existing_criteria = ThesisCriteria()
        ...
        with cc1:
            ...
            min_growth = st.number_input(
                "Min Revenue Growth (%)", value=existing_criteria.min_growth_pct, step=5.0,
            )
            min_margin = st.number_input(
                "Min Gross Margin (%)", value=existing_criteria.min_gross_margin_pct, step=5.0,
            )
        with cc2:
            min_nrr = st.number_input(
                "Min Net Retention (%)", value=existing_criteria.min_net_retention_pct, step=5.0,
            )
